...
df_leagues = pd.read_csv(r'{}/Leagues_soccervista.csv'.format(LEAGUES_LIST_PATH),sep=';',header=None)
...
for i in df_leagues.index:
    ...
    page = requests.get("https://www.newsoccervista.com/table-{}-{}-soccer-results-and-prediction.html".format(country,league), headers=HEADERS)
    page_content = BeautifulSoup(page.content, "html.parser")
    
    partite = str(page_content.findAll('div',attrs={"class":"hometeam"})).encode("utf-8")
    ...
    try:
        if( actual_hash != mycursor.fetchall()[0][0] ):
            ...
            Soccervista.execute(page_content,
                                competition_cod)

            mycursor.execute("REPLACE INTO {}.hash_table_soccervista \
                              SET competition_cod = '{}', hash = '{}'".format(DB_SCHEMA, 
                                                                              competition_cod, 
                                                                              actual_hash))
            mydb.commit()            
    except Exception as e:
    ...
    Utility.telegram_bot_sendlog("END Update Soccervista predictions {} {}".format(country,league))
